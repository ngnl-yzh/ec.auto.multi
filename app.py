    ...
    with tab1:
        ...
        with left:
            ...
            with st.expander("⏰ 지정 시간 추가/관리"):
                    ...
                    <div class="info-box">
                    💡 <b>수집 범위 안내</b><br>
                    각 지정 시간은 <b>설정한 시간 범위만큼 이전부터 현재까지</b> 수집합니다.<br>
                    예) 12시 수집(5시간) → 7시~12시 / 12시 1분 수집(12시간) → 0시 1분~12시 1분<br>
                    시간대가 겹치는 경우 <b>중복 URL은 자동으로 건너뜁니다.</b>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    for user in users:
        ...
        with st.expander(f"{user['email']}  —  {role_labels.get(user['role'], user['role'])}"):
            ...
            with col1:
                ...
                b_brf_std   = user.get("briefing_std_bonus", 0)
                b_brf_det   = user.get("briefing_det_bonus", 0)
                ...
                if b_brief:    bonus_items.append(f"브리핑(공용) +{b_brief}")
                if b_brf_std:  bonus_items.append(f"브리핑기본 +{b_brf_std}")
                if b_brf_det:  bonus_items.append(f"브리핑상세 +{b_brf_det}")
